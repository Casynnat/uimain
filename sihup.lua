function GetUi() 
    if getgenv().Tvk then
    if game.CoreGui:FindFirstChild("Shinichii Hub GUI") then
        for a, v in ipairs(game.CoreGui:GetChildren()) do
            if v.Name == "Shinichii Hub GUI" then
                v:Destroy()
            end
        end
    end
...
function SaveCustomUISettings()
    local j = game:GetService("HttpService")
    if not isfolder("Shinichii Hub") then
        makefolder("Shinichii Hub")
    end
    writefile("Shinichii Hub/" .. k, j:JSONEncode(CorrectTable(e)))
end
function ReadCustomUISetting()
    local l, m =
        ...
        function()
            ...
            local n = j:JSONDecode(readfile("Shinichii Hub/" .. k))
...
u.NotiGui.Name = "Shinichii Hub Notification"
...
function t.CreateNoti(H)
    ...
    local g = V .. "," .. W .. "," .. X
    Q.Text = '<font color="rgb(' .. g .. ')">Shinichii Hub</font> ' .. getgenv().TitleNameNoti
    table.insert(
        c["Title Text Color"],
        function()
            local V = tostring(u.Getcolor(getgenv().UIColor["Title Text Color"])[1])
...
function t.CreateMain(H)
    local Z = tostring(H.Title) or "Shinichii Hub"
    ...
    ImageButton.Image = "rbxassetid://78774998900413"
    ...
    function u.ReloadMain(v)
        ...
        local X = tostring(u.Getcolor(getgenv().UIColor["Title Text Color"])[3])
        local g = V .. "," .. W .. "," .. X
        ad.Text = '<font color="rgb(' .. g .. ')">Shinichii Hub</font> ' .. getgenv().MainDesc
        table.insert(
            c["Title Text Color"],
            function()
                aa.ImageColor3 = getgenv().UIColor["Title Text Color"]
